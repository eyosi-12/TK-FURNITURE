...
const Products = () => {
  ...
  }, [products, selectedRoom, selectedSubTypes, selectedMaterials, maxPrice, searchQuery]);
  ...
  const filterSidebar = (
    <div className="bg-white rounded-2xl border border-[#E7E2D9] p-6 space-y-6 shadow-xs">
      ...
      {/* Item Type / Sub-category Checkboxes - Wireframe Figure 6 */}
      ...
      {/* Price Range Filter - Wireframe Figure 6 */}
      <div className="space-y-3 pt-2 border-t border-[#F3EFEA]">
        <div className="flex items-center justify-between text-xs">
          ...
          <span className="font-bold text-[#1C1917]">
            Up to ETB {maxPrice.toLocaleString()}
          </span>
        </div>
        <input
          type="range"
        ...
        <div className="flex justify-between text-[11px] text-[#A8A29E]">
          ...
          <span>ETB 70,000</span>
        </div>
      </div>

      {/* Material Filter - Wireframe Figure 6 */}
      <div className="space-y-2 pt-2 border-t border-[#F3EFEA]">
        <label className="text-xs font-bold uppercase tracking-wider text-[#78716C] block">
          Material
        </label>
      ...
      {/* Apply Filters Action Button - Wireframe Figure 6 */}
      <button
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-14">
      {/* Breadcrumb Navigation - Wireframe Figure 6 */}
      <nav className="text-xs text-[#78716C] mb-6 flex items-center gap-2">
        ...
        {selectedRoom !== "All" && (
          <>
            <span>/</span>
            <span className="text-[#1C1917] font-semibold">{selectedRoom}</span>
          </>
        )}
      </nav>

      {/* Category Title Header - Wireframe Figure 6 */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-8 mb-8 border-b border-[#E7E2D9]">
        <div>
          <h1 className="text-3xl sm:text-4xl font-bold text-[#1C1917]">
            {selectedRoom === "All" ? "Complete Catalog" : selectedRoom}
          </h1>
          <p className="text-sm text-[#78716C] mt-1">
            Showing {filteredProducts.length} handcrafted furniture pieces
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 items-start">
        ...
        {/* Right-hand Product Grid - Wireframe Figure 6 (3 columns) */}
        <main className="lg:col-span-9 space-y-10">
          {paginatedProducts.length === 0 ? (
            <div className="bg-white rounded-2xl border border-[#E7E2D9] p-12 text-center space-y-3">
              <p className="text-lg font-bold text-[#1C1917]">No pieces matched your selection.</p>
              <p className="text-sm text-[#78716C]">Try widening your price range or clearing active filters.</p>
              <button
                onClick={handleResetFilters}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
              {paginatedProducts.map((product) => (
                <ProductCard key={product.id} item={product} buttonText="View" />
              ))}
            </div>
          )}

          {/* Pagination Controls - Wireframe Figure 6: < 1 2 3 4 > */}
          {totalPages > 1 && (
